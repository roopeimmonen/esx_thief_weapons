	...
	ESX.TriggerServerCallback('esx_thief:getOtherPlayerData', function(data)
		...
		if Config.EnableCash then
			table.insert(elements, {
				label = (('[%s] $%s'):format('Käteinen', ESX.Math.GroupDigits(data.money))),
				value = 'money',
				type = 'item_money',
				amount = data.money
			})
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, Keys['H']) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and IsPedOnFoot(ped) then
